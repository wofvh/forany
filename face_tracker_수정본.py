...
def convert_to_gif (data):  # 잘모르겠다 ㅜ 
	save_dir = "/home/nvidia/dcornic-media/src/gif/"
	os.makedirs(save_dir, exist_ok=True)
	image_paths = [f"/home/nvidia/dcornic-media/src/gif/{i}" for i in os.listdir("/home/nvidia/dcornic-media/src/gif/") if os.path.isfile(f"/home/nvidia/dcornic-media/src/gif/{i}")]
	images = [Image.open(path) for path in image_paths]
	imageio.mimsave(output_file,images, fps=3.5)
	output_file = os.path.join(save_dir, "/home/nvidia/dcornic-media/src/gif/")
...
class FaceTracker:
	...
	def image_to_gif (self, data): # 잘모르겠다 ㅜ 
		self.imagetogif_ = data.data
...
def main():
	...
	imagetogif = rospy.Publisher("/image_to_jeon", Image ,queue_size=10)

	
	# Subscriber
	rospy.Subscriber("/image_to_jeon", Image, ic.image_to_gif)
		...
		if not ic.isSaying_ : 	 # 잘모르겠다 ㅜ 
			...
			if ic.isCaptured_ :
				...
				if capturedCount == 1 :
					...
					cv2.imwrite('/home/nvidia/dcornic-media/src/13.jpg',cv2.cvtColor(imgTemp, cv2.COLOR_BGRA2RGB))
				elif capturedCount == 5 :
					requestShutterSoundPublisher.publish(Empty())
					imgTemp = ic.frontImage_.copy()
					ic.capturedImage_ = ic.frontImage_.copy()
					imgTemp = logoOverlay(imgTemp, dcornicLogoImg)
					cv2.imwrite('/home/nvidia/dcornic-media/src/14.jpg',cv2.cvtColor(imgTemp, cv2.COLOR_BGRA2RGB))
						
				elif capturedCount == 8 :
					requestShutterSoundPublisher.publish(Empty())
					imgTemp = ic.frontImage_.copy()
					ic.capturedImage_ = ic.frontImage_.copy()
					imgTemp = logoOverlay(imgTemp, dcornicLogoImg)
					cv2.imwrite('/home/nvidia/dcornic-media/src/15.jpg',cv2.cvtColor(imgTemp, cv2.COLOR_BGRA2RGB))
				elif capturedCount == 12 :
					requestShutterSoundPublisher.publish(Empty())
					imgTemp = ic.frontImage_.copy()
					ic.capturedImage_ = ic.frontImage_.copy()
					imgTemp = logoOverlay(imgTemp, dcornicLogoImg)
					cv2.imwrite('/home/nvidia/dcornic-media/src/16.jpg',cv2.cvtColor(imgTemp, cv2.COLOR_BGRA2RGB))
					
     
     
				elif capturedCount == 16 :
					requestImageHostingPublisher.publish(Empty())
				elif capturedCount == 37 :
