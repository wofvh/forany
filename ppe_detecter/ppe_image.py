...
def detect_ppe(image, model, timestamp):
    ...
    if np.shape(results.xyxy[0].cpu().numpy())[0] > 0:
        ...
        cv2.putText(image, f"[Missing {missing_hooks} hooks]", (150, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.4, color_hooks, 2)
        cv2.putText(image, f"[Missing {missing_helmet} helmet]", (150, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.4, color_helmet, 2)
...
image_folder_path = "D:/logic/Simplatform/images/"  # 입력 이미지가 있는 폴더 경로
output_folder_path = "D:/logic/Simplatform/output/"  # 결과 이미지를 저장할 폴더 경로
ppe_model = torch.hub.load('WongKinYiu/yolov7', 'custom', './PPE/best_410.pt', force_reload=False)
# 폴더 내 모든 이미지 파일을 처리
for filename in os.listdir(image_folder_path):
    if filename.endswith(('.jpg', '.jpeg', '.png')):  # 이미지 파일 확장자 확인
        image_path = os.path.join(image_folder_path, filename)
        output_path = os.path.join(output_folder_path, f"result_{filename}")  # 결과 이미지를 저장할 경로
        ...
        frame = cv2.resize(frame, (480, 480))  # 이미지 크기 조정
