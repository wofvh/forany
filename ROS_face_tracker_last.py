...
def main():
	...
	makevideo_ = rospy.Publisher('/makevideo', Empty, queue_size = 1)
	...
	rospy.Subscriber("/life_shot_image_", Int32, ic.imagelifeshot)
	...
	font1 = ImageFont.truetype("/home/nvidia/dcornic-media/src/NanumSquare_acB.ttf", 17)
	text = u"동영상을 촬영 중입니다"
	text1 = u"찰칵 소리가 나면 포즈를 바꿔보세요"
		...
		if not ic.isSaying_ :
			...
			if ic.isCaptured_ :
				...
				if capturedCount <= capturedCount_:
					...
					draw.text((390, 100), text, (255,255,255),font = font)
					draw.text((370, 130), text1, (255,255,255),font = font1)
					addv = np.array(to_pil)
					imgTemp = ic.frontImage_.copy()
					imgTemp = logoOverlay(imgTemp, dcornicLogoImg)
					# numpy_image = np.array(to_pil)
					imgTemp = cv2.cvtColor(imgTemp, cv2.COLOR_RGB2BGR)
					...
					elif capturedCount == 45:
						requestShutterSoundPublisher.publish(Empty())
					elif capturedCount == 90:
						requestShutterSoundPublisher.publish(Empty())
					elif capturedCount == 135:
						requestShutterSoundPublisher.publish(Empty())
    
				elif capturedCount == capturedCount_ + 4 :
					ic.capturedImage_ = ic.frontImage_.copy()
					makevideo_.publish(Empty())
					# requestImageHostingPublisher.publish(Empty())
				elif capturedCount == capturedCount_ + 20 :
			...
			if ic.isRequestedQr_ :
				if ic.qrCount_ == 0 :
					...
					croppedCaturedImage = cv2.cvtColor(cv2.imread("/home/nvidia/dcornic-media/src/video/save/30.jpg"), cv2.COLOR_BGR2RGB)[0:480, 80:560]
